    ...
    <div className="bg-card border border-line rounded-xl p-4 space-y-3">
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-xs text-muted mt-0.5">{productLabel[stock.product]}</p>
        </div>
      ...
      <div>
        ...
        <div className="h-3 bg-card-2 rounded-full overflow-hidden">
          <div className={`h-full ${barColor} rounded-full transition-all duration-700`} style={{ width: `${pct}%` }} />
        </div>
      </div>
      <div className="flex justify-between text-xs text-muted">
        <span>Price: <span className="text-foreground font-semibold">{stock.price}/L</span></span>
        <span>Restocked: <span className="text-foreground">
...
const inputCls = "w-full bg-card/60 border border-line rounded-lg px-3 py-2 text-foreground placeholder-muted focus:outline-none focus:border-orange-500 focus:ring-1 focus:ring-orange-500 transition text-sm";
const selectCls = "w-full bg-card/60 border border-line rounded-lg px-3 py-2 text-foreground focus:outline-none focus:border-orange-500 focus:ring-1 focus:ring-orange-500 transition text-sm";
...
export default function StationManager() {
    ...
    <div className="min-h-screen text-foreground">
      ...
      <div className="relative z-10 pt-16 md:pl-64 flex items-center justify-center min-h-screen">
        <div className="text-center text-muted p-8">
          <p className="text-lg font-semibold text-foreground mb-2">No stations registered</p>
  ...
  const badge = (s: string) => {
    const map: Record<string, string> = {
      ...
      inactive:    "bg-gray-500/20 text-muted border-line/40",
      ...
      Critical:    "bg-orange-500/20 text-orange-400 border-orange-500/40",
      Empty:       "bg-red-500/20 text-red-400 border-red-500/40",
      "Off Duty":  "bg-gray-500/20 text-muted border-line/40",
      "On Leave":  "bg-blue-500/20 text-blue-400 border-blue-500/40",
    };
    return (map[s] ?? "bg-gray-500/20 text-muted border-line/40") + " px-2 py-0.5 rounded-full text-xs font-bold border";
  };
    ...
    >
      ...
      {editOpen && editDraft && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative z-10 w-full max-w-xl bg-background border border-line rounded-2xl shadow-2xl overflow-hidden">
            <div className="flex items-center justify-between px-6 py-4 border-b border-line bg-orange-500/5">
              ...
              <button onClick={() => setEditOpen(false)} className="text-muted hover:text-foreground transition">
            ...
            <div className="p-6 space-y-4 max-h-[75vh] overflow-y-auto">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-xs font-semibold text-muted uppercase tracking-wider mb-1">Station Status</label>
            ...
            <div className="px-6 py-4 border-t border-line space-y-3">
              ...
              <div className="flex justify-end gap-3">
                <button onClick={() => { setEditOpen(false); setEditError(""); }}
                  className="px-4 py-2 border border-line text-muted hover:text-foreground text-sm font-semibold rounded-lg transition">
                  Cancel
                </button>
      ...
      {reportOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative z-10 w-full max-w-2xl bg-background border border-line rounded-2xl shadow-2xl overflow-hidden">
            <div className="flex items-center justify-between px-6 py-4 border-b border-line bg-orange-500/5">
              <div>
                <p className="text-sm font-bold text-orange-400 uppercase tracking-wider">Daily Station Report</p>
                <p className="text-xs text-muted mt-0.5">{station.name} · {report.date}</p>
              </div>
              <button onClick={() => setReportOpen(false)} className="text-muted hover:text-foreground transition">
                <svg className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" />
                </svg>
              </button>
            ...
            <div className="p-6 space-y-5 max-h-[80vh] overflow-y-auto">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                {[
                  ...
                  <div key={c.label} className="bg-card border border-line rounded-xl p-3">
              ...
              <div className="bg-card border border-line rounded-xl overflow-hidden">
                <div className="px-4 py-2.5 border-b border-line bg-orange-500/5">
                  ...
                  <table className="w-full">
                    <thead>
                      <tr className="border-b border-line">
                        {["Product", "Opening Stock", "Qty Sold", "Remaining", "Fill %", "Transactions", "Revenue"].map((h) => (
                          <th key={h} className="px-4 py-2.5 text-left text-xs font-bold text-muted uppercase tracking-wider whitespace-nowrap">{h}</th>
                        ))}
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-line/60">
                      {report.rows.map((r) => {
                        ...
                        return (
                          <tr key={r.product} className="hover:bg-card-2 transition-colors">
                            <td className="px-4 py-3">
                              <span className={`font-extrabold text-sm ${prodColor(r.product)}`}>{r.product}</span>
                            </td>
                            <td className="px-4 py-3 text-xs text-muted whitespace-nowrap">{(r.remaining + r.sold).toLocaleString()} L</td>
                            <td className="px-4 py-3 text-sm font-semibold text-foreground whitespace-nowrap">{r.sold.toLocaleString()} L</td>
                            <td className="px-4 py-3 text-xs text-foreground whitespace-nowrap">{r.remaining.toLocaleString()} L</td>
                            <td className="px-4 py-3">
                              <div className="flex items-center gap-2">
                                <div className="w-14 h-2 bg-card-2 rounded-full overflow-hidden">
                                  <div className={`h-full ${bar} rounded-full`} style={{ width: `${pct}%` }} />
                                </div>
                                <span className="text-xs text-muted">{pct}%</span>
                              </div>
                            </td>
                            <td className="px-4 py-3 text-sm text-foreground whitespace-nowrap">{r.txns}</td>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="bg-card border border-line rounded-xl p-4 space-y-2">
                  <p className="text-xs font-bold text-muted uppercase tracking-wider mb-2">Operations</p>
                  {[
                    ...
                    <div key={label} className="flex justify-between text-xs border-b border-line/50 pb-1.5 last:border-0 last:pb-0">
                ...
                <div className="bg-card border border-line rounded-xl p-4 space-y-2">
                  <p className="text-xs font-bold text-muted uppercase tracking-wider mb-2">Payment Channels</p>
                  {[
            ...
            <div className="px-6 py-4 border-t border-line flex justify-between items-center">
              <p className="text-xs text-muted">Generated: {new Date().toLocaleTimeString("en-NG")}</p>
              <button
                onClick={() => {
      ...
      <div className="relative z-10 pt-16 md:pl-64 min-h-screen">
        <div className="px-4 sm:px-6 py-6">
          ...
          {/* Header */}
          <div className="mb-6">
            <h1 className="text-3xl font-bold text-foreground mb-1">Station Manager</h1>
            <p className="text-muted text-sm">Manage your fuel stations, inventory, and personnel</p>
          </div>
              ...
              <button
                ...
                className={`flex items-center gap-2 px-4 py-2.5 rounded-xl border text-sm font-semibold transition ${
                  i === activeIdx
                    ...
                    : "bg-card backdrop-blur-md border-line text-foreground hover:border-orange-500/50 hover:text-orange-400"
                }`}
              >
          ...
          {/* Station banner */}
          <div className="bg-card backdrop-blur-md border border-line rounded-xl p-4 mb-6 flex flex-col sm:flex-row sm:items-center gap-4">
            <div className="flex-1 min-w-0">
              ...
              <p className="text-sm text-muted truncate">{station.address}, {station.state} · {station.openingHours}</p>
              <p className="text-xs text-muted mt-0.5">Licence: {station.licenseNo} · DPR: {station.dprNo}</p>
            </div>
            <div className="flex gap-2 flex-wrap flex-shrink-0">
              ...
              <button
                onClick={openEdit}
                className="flex items-center gap-1.5 px-4 py-2 bg-card-2 hover:bg-card-2 border border-line hover:border-orange-500/50 text-foreground hover:text-orange-400 text-xs font-semibold rounded-lg transition"
              >
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
            {[
              ...
              <div key={c.label} className={`bg-card backdrop-blur-md border ${c.border} rounded-lg p-4`}>
          ...
          {/* Tab bar */}
          <div className="flex gap-1 mb-6 bg-card backdrop-blur-sm border border-line rounded-xl p-1 w-fit overflow-x-auto">
            {TABS.map((t) => (
              <button
                key={t.id}
                onClick={() => setTab(t.id)}
          ...
          {tab === "overview" && (
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <div className="bg-card backdrop-blur-md border border-line rounded-xl p-5 space-y-4">
                <p className="text-xs font-bold text-muted uppercase tracking-wider">Inventory Snapshot</p>
                {station.stock.map((s) => <StockGauge key={s.product} stock={s} />)}
              </div>
              <div className="space-y-4">
                <div className="bg-card backdrop-blur-md border border-line rounded-xl p-5 space-y-2.5">
                  <p className="text-xs font-bold text-muted uppercase tracking-wider mb-1">Station Details</p>
                  {[
                ...
                <div className="bg-card backdrop-blur-md border border-line rounded-xl p-5">
                  <p className="text-xs font-bold text-muted uppercase tracking-wider mb-3">Latest Supply Activity</p>
                  <div className="space-y-3">
                    {station.supplies.slice(0, 4).map((s) => (
                      <div key={s.id} className="flex items-center justify-between py-1 border-b border-line/50 last:border-0">
                        <div>
            ...
            <div className="space-y-6">
              ...
              <div className="bg-card backdrop-blur-md border border-line rounded-xl overflow-hidden">
                <div className="px-5 py-3 border-b border-line flex items-center justify-between">
                  <p className="text-xs font-bold text-muted uppercase tracking-wider">Detailed Inventory</p>
                  <button
                    onClick={() => router.push(`/customer/request-supply?station=${station.id}&name=${encodeURIComponent(station.name)}`)}
                  ...
                  <table className="w-full">
                    ...
                    <tbody className="divide-y divide-line">
                      {station.stock.map((s) => {
                        ...
                        const bar = pct > 50 ? "bg-green-500" : pct > 20 ? "bg-yellow-500" : "bg-red-500";
                        return (
                          <tr key={s.product} className="hover:bg-card-2 transition-colors">
                            <td className="px-4 py-3"><span className={`font-bold text-sm ${prodColor(s.product)}`}>{s.product}</span></td>
                            <td className="px-4 py-3 text-sm text-foreground">{s.current.toLocaleString()} L</td>
                            <td className="px-4 py-3 text-sm text-muted">{s.capacity.toLocaleString()} L</td>
                            <td className="px-4 py-3">
                              <div className="flex items-center gap-2">
                                <div className="w-20 h-2 bg-card-2 rounded-full overflow-hidden">
                                  <div className={`h-full ${bar} rounded-full`} style={{ width: `${pct}%` }} />
                                </div>
                                <span className="text-xs text-muted w-8">{pct}%</span>
                              </div>
                            </td>
                            <td className="px-4 py-3 text-sm text-foreground font-semibold">{s.price}</td>
            ...
            <div className="bg-card backdrop-blur-md border border-line rounded-xl overflow-hidden">
              ...
              <div className="overflow-x-auto">
                <table className="w-full">
                  ...
                  <tbody className="divide-y divide-line">
                    {station.staff.map((s) => (
                      <tr key={s.id} className="hover:bg-card-2 transition-colors">
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-2">
                        ...
                        <td className="px-4 py-3 text-sm text-foreground whitespace-nowrap">{s.role}</td>
                        <td className="px-4 py-3 text-xs text-muted whitespace-nowrap">{s.shift}</td>
                        <td className="px-4 py-3 text-xs text-muted whitespace-nowrap">{s.phone}</td>
          ...
          {tab === "supplies" && (
            ...
            <div className="bg-card backdrop-blur-md border border-line rounded-xl overflow-hidden">
              <div className="px-5 py-3 border-b border-line bg-orange-500/5 flex items-center justify-between">
                ...
                <span className="text-xs text-muted">{depots.length} depots</span>
              </div>
              <div className="overflow-x-auto">
                <table className="w-full">
                  <thead>
                    <tr className="border-b border-line bg-card/30">
                      <th className="px-4 py-2.5 text-left text-xs font-bold text-muted uppercase tracking-wider whitespace-nowrap">Depot</th>
                      {(["PMS", "AGO", "ATK"] as ProductKey[]).map((pk) => (
                        <th key={pk} className="px-4 py-2.5 text-left text-xs font-bold text-muted uppercase tracking-wider whitespace-nowrap">{pk}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-line/50">
                    {depots.map((depot) => (
                      <tr key={depot} className="hover:bg-card-2 transition-colors">
                        <td className="px-4 py-3 text-sm text-foreground font-medium whitespace-nowrap">{depot}</td>
                        {(["PMS", "AGO", "ATK"] as ProductKey[]).map((pk) => {
                          const d = depotProducts[depot]?.[pk];
                          if (!d) return <td key={pk} className="px-4 py-3 text-xs text-muted">—</td>;
                          ...
                          return (
                            <td key={pk} className="px-4 py-3">
                              <div className="flex items-center gap-2 min-w-[110px]">
                                <div className="w-16 h-1.5 bg-card-2 rounded-full overflow-hidden shrink-0">
            ...
            <div className="bg-card backdrop-blur-md border border-line rounded-xl overflow-hidden">
              <div className="px-5 py-3 border-b border-line flex items-center justify-between">
                <p className="text-xs font-bold text-muted uppercase tracking-wider">Supply History</p>
                <button
                  onClick={() => router.push(`/customer/request-supply?station=${station.id}&name=${encodeURIComponent(station.name)}`)}
                  className="px-3 py-1.5 bg-orange-500 hover:bg-orange-600 text-white text-xs font-bold rounded-lg transition"
                >
                  ...
                  <tbody className="divide-y divide-line">
                    {station.supplies.map((s) => (
                      <tr key={s.id} className="hover:bg-card-2 transition-colors">
                        ...
                        <td className="px-4 py-3 text-xs text-muted">{s.depot}</td>
                        <td className="px-4 py-3 text-xs text-muted font-mono">{s.truckReg}</td>
                        <td className="px-4 py-3"><span className={badge(s.status)}>{toLabel(s.status)}</span></td>
                      </tr>
                    ))}
                  </tbody>
